...
// Splits "R 2,967 /mo" / "Valued at R 2,000 /mo" / "No Charge" into a headline amount and a caption
const splitPrice = (price) => {
  const monthly = /\/mo/i.test(price);
  const valued = /valued/i.test(price);
  const amount = price.replace(/valued at/i, '').replace(/\/mo/i, '').trim();
  let caption = 'Once-off';
  if (valued) caption = 'Added value per month';
  else if (monthly) caption = 'Per month';
  else if (/no charge/i.test(price)) caption = 'Included in rollout';
  return { amount, caption };
};

const FeatureSection = ({ data }) => {
  const showPrice = data.price && !data.hidePriceBlock;
  const priceParts = showPrice ? splitPrice(data.price) : null;
          ...
          >
            ...
            {priceParts && (
              <div className="feature-card-bar">
                <span className="feature-card-bar-amount">{priceParts.amount}</span>
                <span className="feature-card-bar-caption">{priceParts.caption}</span>
              </div>
            )}
          </motion.div>
